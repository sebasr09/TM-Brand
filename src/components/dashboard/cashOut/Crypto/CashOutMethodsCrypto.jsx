...
import BancolombiaLogo from '../../../../logos/bancolombia-logo.png';
...
export default function CashInTextField({mainText, value, handleBoughtInfoChange}) {
  ...
  const methods = [{logo: BancolombiaLogo, title: "Bancolombia"}, {logo: ClaroPayLogo, title: "Claro Pay"}]

  const classes = useStyles(); 
  return (
      <ImageList cols={2}>
        {methods.map((item, index) => (
          <ImageListItem key={index} style={{height:50+'%'}}>
            <img
              src={item.logo}
              srcSet={item.logo}
              alt={item.title}
