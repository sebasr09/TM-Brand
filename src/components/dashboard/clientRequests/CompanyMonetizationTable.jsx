...
import EnhancedTableHead from './companyMonetizationTableComponents/EnhancedTableHead';
...
export default function EnhancedTable({ user, userInfo }) {
  ...
  const getRequests = async (limitParam, offsetParam) => {
    ...
    try {
      ...
      const data = await getApi(`activos/getAllCompanyMonetizations/desc/${offset}/${limit}`);
  ...
  return (
    <div className={classes.root}>
      ...
      <MonetizationCreate open={openCreateMonetization} handleModifyClose={handleModifyCloseMonetization} userInfo={userInfo} setInformationMessage={setInformationMessage} setOpenInformation={setOpenInformation}  request={selectedMonetization} isEdit={isEdit} isMonetization={true}/>
      <React.Fragment>
                  ...
                  requests.map((row, index) => {
                    return (
                      <TableRow hover role='checkbox' tabIndex={-1} key={index} id={`userDetail${index}`}>
                        ...
                        <TableCell>{row.company_origin.name}</TableCell>
                        <TableCell>{row.company_final.name}</TableCell>
                        <TableCell>{row.id_monetization.id_transaction.id_origin_coin_coin.short_name + " $" + formatNumber(row.id_monetization.id_transaction.origin_value)}</TableCell>
                        <TableCell>{row.id_monetization.id_transaction.id_final_coin_coin.short_name + " $" + formatNumber(row.id_monetization.id_transaction.final_value)}</TableCell>
                        ...
                        <TableCell>{row.id_monetization.id_transaction.rate}</TableCell>
                        <TableCell>{row.concept}</TableCell>
                        <TableCell>{row.imc.name}</TableCell>
