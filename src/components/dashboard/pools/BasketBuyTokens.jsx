...
export default function BasketBuyTokens({ open, basket, handleClose }) {
  ...
  const buyMultipleTokens = async () => {
    try {
      setOpenBackdrop(true)
      await postApi('next-activos/buyMultipleTokens', { tokenBuys: ids, basketId: basket.token.id })
      setSuccessMessage('Se completó el ingreso de tokens.')
      setOpenSuccess(true);
      handleClose()
    } catch (err) {
      setOpenError(true);
      setErrorMessage('No fue posible ingresar los tokens.');
    } finally {
  ...
  /* const setBasketEntriesData = () => {
      request && request.coins && setIds(request.coins.map((e) => {
          return {
              token: e,
              aditional: false,
              krakenTxId: '',
  ...
  return (
    <Dialog open={open} onClose={handleClose} TransitionComponent={Transition} keepMounted>
      ...
      <Card className={classes.root} sx={{ overflow: 'overlay' }}>
        <CardContent>
          <PagesHeaderBar
            title='Ingresar Tokens'
            subTitle='Selecciona las monedas que quieras ingresar.'
          />
          <Divider />
