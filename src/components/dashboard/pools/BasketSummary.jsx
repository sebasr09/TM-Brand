...
import { ExpandLess, ExpandMore, Pool } from '@mui/icons-material';
...
export default function InvestmentView({ basketInfo, setUserInfo, userInfo }) {
  ...
  const getTotal = () => {
    ...
    setUserInfo((previous) => {
      ...
      newObject.balance = newObject.balance? newObject.balance + total : total;
      return newObject;
    });
    setTotal(total);
  };
  ...
  const totalChange = () => {
    let totalN = 0;
    cryptosList.forEach((crypto) => {
      if (crypto.available && parseFloat(crypto.available) > 0) {
        totalN += parseFloat(crypto.initial);
      }
    });
      ...
      <Stack sx={{ margin: '1rem 0.5em 0 0.5em' }}>
        <Stack direction='row' justifyContent='space-between'>
          <Typography variant='h5' color={Colors.light.primary}>
            <Pool fontSize='small' /> <strong>{basketInfo.basket.name}</strong>
          </Typography>
          <Typography
            variant='button'
            fontWeight='bold'
            color={`${change > 0 ? Colors.light.green : Colors.light.red}`}
          >
            {formatNumber(change)} %
          </Typography>
        </Stack>
        ...
        >
          ...
          <Typography color={Colors.light.primary}>
            {collapse ? (
      ...
      <Stack>
        <Collapse in={collapse}>
          ...
          <Doughnut
            ...
            autoSave={true}
          />
          <List sx={{ overflow: 'auto', maxHeight: { sm: '100', md: '50vh' } }}>
                ...
                <div key={index}>
                  <ListItem sx={{ paddingLeft: 0 }}>
                    ...
                    <Stack
                      ...
                      sx={{
                        display: 'contents',
                        color: `${row.change > 0 ? Colors.light.green : Colors.light.red}`,
                      }}
                    >
                      <ListItemText
                        sx={{ textAlign: 'right' }}
